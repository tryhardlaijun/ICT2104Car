...
coordinate initStartingCoordinate(){
    coordinate startingCoordinate = {0,0,0,0,0,0,1};
...
void printCoordinate(coordinate c){
    printf("x is %d\ty is %d\tpathAvail is %d\tpathUnexplored is %d\tNext orientation is %d\tSelf Orientation is %d\tisLast is %d\t\n"
    ,c.x,c.y,c.pathAvail,
    c.pathUnexplored,c.nextOrientation,c.selfOrientation,c.isLast);
}
